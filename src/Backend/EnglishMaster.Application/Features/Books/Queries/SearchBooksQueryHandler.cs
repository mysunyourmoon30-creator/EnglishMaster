...
public sealed class SearchBooksQueryHandler
{
    ...
    private const int MaximumSearchLength = BookFieldLimits.Title;
    ...
    public async Task<Result<BookSearchResponse>> HandleAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sortBy = ParseOptionalEnum(query.SortBy, nameof(query.SortBy), BookSortBy.Title, errors);
        ...
        if (errors.Count > 0)
        {
            return Result<BookSearchResponse>.Validation([.. errors]);
        }
        ...
        var searchResult = await bookRepository.SearchAsync(criteria, cancellationToken);
        var items = await BookReadModelBuilder.MapAsync(
            searchResult.Items,
            categoryRepository,
            mediaRepository,
            courseRepository,
        ...
        return Result<BookSearchResponse>.Success(new BookSearchResponse(
            items,
            pageNumber,
            pageSize,
        ...
        ICollection<ValidationError> errors)
    {
        ...
        if (value.Value < 1)
        {
            errors.Add(new ValidationError(
                nameof(SearchBooksQuery.PageNumber),
                $"{nameof(SearchBooksQuery.PageNumber)} must be greater than or equal to 1."));
            return DefaultPageNumber;
        }
        ...
        ICollection<ValidationError> errors)
    {
        ...
        {
            errors.Add(new ValidationError(
                nameof(SearchBooksQuery.PageSize),
                $"{nameof(SearchBooksQuery.PageSize)} must be between 1 and {MaximumPageSize}."));
            return DefaultPageSize;
        }
        ...
        ICollection<ValidationError> errors)
    {
        ...
        {
            errors.Add(new ValidationError(
                nameof(SearchBooksQuery.Search),
                $"{nameof(SearchBooksQuery.Search)} must be {MaximumSearchLength} characters or fewer."));
        }
