...
internal static class CourseInputValidator
{
    public static Result<CourseInput> Validate(
        string? title,
        string? summary,
        string? description,
        string? cefrLevel,
        ...
        bool isActive)
    {
        ...
        var normalizedTitle = NormalizeRequired(title, nameof(title), CourseFieldLimits.Title, errors);
        var normalizedSlug = NormalizeSlug(normalizedTitle, nameof(title), errors);
        var normalizedSummary = NormalizeOptional(summary, nameof(summary), CourseFieldLimits.Summary, errors);
        var normalizedDescription = NormalizeOptional(description, nameof(description), CourseFieldLimits.Description, errors);
        ...
        if (errors.Count > 0)
        {
            return Result<CourseInput>.Validation([.. errors]);
        }

        return Result<CourseInput>.Success(new CourseInput(
            normalizedTitle,
            normalizedSlug,
            normalizedSummary,
        ...
        ICollection<ValidationError> errors)
    {
        ...
        try
        {
            return Course.GenerateSlug(normalizedTitle);
        }
